...
2. **Gates are blocking.** ESLint and Jest run before every commit. If either fails, attempt auto-fix once (`eslint --fix`), rerun, and if it still fails, STOP that repo — do NOT commit broken code. Mark the repo as **blocked**, report it, move on.

3. **One feature per commit by default.** Only lump features into a single commit/PR when the user has explicitly said "these can be pushed and lumped together" (or equivalent). Otherwise: one feature → one commit → one PR.

4. **Correct order matters.** Commit in dependency order: config/types → core libs → consumers → tests/docs. If feature B imports from feature A, A commits first.

5. **Blocked ≠ stopped.** When a repo is blocked, alert the user in **bold** with the repo name and reason. Then keep going — other repos in the run continue in parallel. Never abandon the whole run for a single repo's blocker.

6. **Detect, don't assume.** The target may be a single repo (`.git` at root), a folder of repos (multiple subdirs with `.git`), or `.` itself. Detect via `scripts/detect_repos.sh` before doing anything else.
...
5. EACH SUBAGENT runs:  branch → gate → commit → push → PR → self-review → post
...
For EACH feature, in order, do:
  1. Stage exactly that feature's files: `git add <files>` (no `git add .`)
  2. Run gates from the orchestrator's skill scripts:
  ...
  3. Create branch: feat/<feature-name> off latest base branch (rebase if needed).
  4. Commit: `git commit -m "<type>(<scope>): <summary>"` — clean message, no trailers.
  5. Push: `git push -u origin feat/<feature-name>`
  6. Detect host: orchestrator passes HOST=github|gitlab based on remote URL.
  7. Build PR body from template at <skill-path>/references/pr-description-template.md, filled with feature data.
  8. Open PR:
  ...
  9. Self-review using <skill-path>/references/pr-review-checklist.md against the diff. Produce two artifacts:
  ...
  11. Write SUCCESS.md with: PR URL, commit SHA, branch name, gate results, review summary.
...
**5b. Stage exact files for this feature**
...
**5d. Commit (clean message, no trailers)**
